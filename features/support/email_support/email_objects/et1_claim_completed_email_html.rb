...
module EtFullSystem
  module Test
    class Et1ClaimCompletedEmailHtml < Base
      ...
      element(:claim_submitted_element, :xpath, XPath.generate do |x|
        x.descendant(:p)[x.string.n.starts_with(t('claim_confirmations.show.header'))]
      end)
      ...
      def submission_submitted
        claim_submitted_element.text.gsub(/#{t('claim_confirmations.show.header')}:/, '').gsub(
          /#{t('base_mailer.confirmation_email.details.submitted_on_prefix')}/, ''
        ).strip
      end

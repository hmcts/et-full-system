    ...
    module FileObjects
      class Et3PdfFile < Base
        ...
        def has_correct_contents_for?(response:, respondent:, representative:, errors: [], indent: 1) # rubocop:disable Naming/PredicateName
          ...
          Et3PdfFileSection::EmploymentDetailsSection.new(tempfile,
                                                          locale: locale).has_contents_for?(response: response)
          ...
          Et3PdfFileSection::ContractClaimSection.new(tempfile,
                                                      locale: locale).has_contents_for?(respondent: respondent)
          Et3PdfFileSection::RepresentativeSection.new(tempfile,
                                                       locale: locale).has_contents_for?(representative: representative)
        ...
        def has_correct_contents_from_db_for?(response:, errors: [], indent: 1)
          ...
          unless representative.nil?
            representative[:address_attributes] =
              representative.delete(:address).symbolize_keys
          end
          ...
          has_correct_contents_for?(response: response, respondent: respondent, representative: representative,
                                    errors: errors, indent: indent)
        end

        ...
        class RespondentsDetailsSection < EtFullSystem::Test::FileObjects::Et1PdfFileSection::Base
          def has_contents_for?(respondents:)
            expected_values = {
              ...
              address: {
                details: [respondents.first.building, respondents.first.street, respondents.first.locality,
                          respondents.first.county].reject(&:blank?).join("\n"),
              ...
              different_address: {
                details: [respondents.first.work_building, respondents.first.work_street,
                          respondents.first.work_locality, respondents.first.work_county].reject(&:blank?).join("\n"),
              ...
              respondent2: {
                ...
                address: {
                  details: [respondents[1].try(:building), respondents[1].try(:street),
                            respondents[1].try(:locality), respondents[1].try(:county)].reject(&:blank?).join("\n"),
              ...
              respondent3: {
                ...
                address: {
                  details: [respondents[2].try(:building), respondents[2].try(:street),
                            respondents[2].try(:locality), respondents[2].try(:county)].reject(&:blank?).join("\n"),

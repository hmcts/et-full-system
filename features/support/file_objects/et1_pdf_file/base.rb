        ...
        class Base < ::EtFullSystem::Test::FileObjects::BasePdfFile
          ...
          def field_value_for(value, key:, path:)
            if value.key?(:select_values)
              ...
              elsif value[:unselected_value] == raw
                return nil
              end
          ...
          def outcome_type_for(outcome_type)
            ...
            when :compensation_only
              :prefer_compensation
            when :tribunal_recommendation
              :prefer_recommendation
            when :reinstated_employment_and_compensation
              :prefer_re_instatement
            when :new_employment_and_compensation

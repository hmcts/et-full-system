        ...
        class YourDetailsSection < EtFullSystem::Test::FileObjects::Et1PdfFileSection::Base
          def has_contents_for?(claimant:, claim:)
            expected_values = {
              ...
              address: [claimant.building, claimant.street, claimant.locality,
                        claimant.county].reject(&:blank?).join("\n"),
              ...
              allow_phone_attendance: claimant.allow_phone_or_video_attendance.map do |option|
                option.to_s.split('.').last
              end.include?('phone'),
              allow_video_attendance: claimant.allow_phone_or_video_attendance.map do |option|
                option.to_s.split('.').last
              end.include?('video'),
              no_phone_or_video_attendance: claimant.allow_phone_or_video_attendance.map do |option|
                option.to_s.split('.').last
              end.include?('neither'),
              case_heard_by_preference: claim.case_heard_by_preference.to_s.split('.').last,

          ...
          def has_contents_for?(representative:)
            if representative.representative_have == 'Yes'
              expected_values = {
                ...
                address: [representative.building, representative.street, representative.locality,
                          representative.county].compact.join("\n"),
              ...
              if representative.representative_contact_preference.to_s.split('.').last == 'email'
                expected_values[:email_address] =
                  representative.email_address
              end
              if representative.representative_contact_preference.to_s.split('.').last == 'dx_number'
                expected_values[:dx_number] =

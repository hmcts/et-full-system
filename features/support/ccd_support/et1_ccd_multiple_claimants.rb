    ...
    module Ccd
      class Et1CcdMultipleClaimants < Base
        ...
        def assert_primary_claimant(claimant, representative, employment, respondents, reference_number, ccd_office,
                                    claim)
          case_references = response.dig('case_fields', 'caseIdCollection').map do |obj|
            obj.dig('value', 'ethos_CaseReference')
          end
          aggregate_failures 'validating key fields' do
            created_case = ccd.caseworker_search_latest_by_ethos_case_reference(case_references.first,
                                                                                case_type_id: ccd_office)

  ...
  module Test
    module Et1ClaimantType
      def claimant_ind_type(claimant, secondary: false)
        gender = { 'prefer_not_to_say' => 'Not Known', 'male' => 'Male',
                   'female' => 'Female' }[claimant[:gender].to_s.split('.').last]
        {
          ...
          'claimant_date_of_birth' => parse_date(claimant[:date_of_birth])&.strftime('%Y-%m-%d'),
      ...
      def claimant_csv_ind_type(claimant)
        {
          ...
          'claimant_date_of_birth' => parse_date(claimant['Date of birth'])&.strftime('%Y-%m-%d'),
          'claimant_gender' => nil
        }
      end
      ...
      def claimant_other_type(employment, claimant)
        common = if employment[:end_date].nil? || employment[:end_date] == ''
                   {
        ...
        if currently_employed?(employment)
          ...
          employment_terminated?(employment)
          common.merge! \
      ...
      def claimant_type(claimant, secondary: false)
        {
          ...
          'claimant_email_address' => if secondary
                                        nil
                                      else
                                        (claimant[:correspondence].to_s =~ /email\z/ ? claimant[:email_address] : nil)
                                      end,

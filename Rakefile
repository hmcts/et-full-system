...
task :setup_buckets do
  ...
  rescue Seahorse::Client::NetworkingError, Aws::S3::Errors::NotFound, Aws::S3::Errors::Http502Error,
         Aws::Errors::ServiceError
    retry_countdown -= 1
    raise 'Could not connect to the S3 server after 10 retries' if retry_countdown.zero?

    warn 'Retrying connection to S3 server in 30 seconds'
    sleep 30
    retry
  end
end
